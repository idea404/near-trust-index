...
class NearTrustIndex {
  ...
  internalCallback({ accountId }: { accountId: string }): { account_id: string; errors: object[]; index: string | null; timestamp: string | null } {
    ...
    const accountScores: number[] = getRawAccountScoresFromPromises(accountId, this.accountIndexHistoryFailures, callCount, this.whitelist, this.accountResult);
...
function constructPromiseChainFromWhitelist(whitelist: object, accountId: string, gasAmountPerCall: bigint): NearPromise {
  ...
  promise = promise.then(NearPromise.new(near.currentAccountId()).functionCall('internalCallback', JSON.stringify({ accountId }), BigInt(0), gasAmountPerCall));
